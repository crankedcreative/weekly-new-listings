...
def fetch_tsxv_new_listings(seed_limit: int = 250) -> list[dict]:
    """
    Scrapes TSX Venture "LatestCompanyDocuments" page for bulletin links, then
    filters to bulletin pages with 'BULLETIN TYPE: New Listing'.
    """
    html = _get(
        TMX_LATEST_URL,
        params={"BulletinsMode": "on", "GetPage": "LatestCompanyDocuments", "NewsReleases": "off"},
    )
    ...
    for x in links:
        ...
        seen.add(x)
        uniq.append(x)
    links = uniq[:seed_limit]

    out: list[dict] = []
    for link in links:
        # prefer the infoventure host for bulletin contents
